...
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends nano $OPENCLAW_DOCKER_APT_PACKAGES && \
    apt-get clean && \
...
COPY --from=runtime-assets --chown=node:node /app/entrypoint.sh ./entrypoint.sh
